...
export default function Header() {
  ...
  const navLinks = [
    ...
    { href: "/produtos?cat=gadgets", label: "Gadgets" },
    { href: "/produtos?cat=camping", label: "Camping" },
    { href: "/produtos?cat=automacao", label: "Casa Inteligente" },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <Link href="/" className="flex items-center gap-2 group">
              <div className="bg-amber-500 rounded-lg p-1.5 group-hover:bg-amber-400 transition-colors">
                <PackageCheck size={18} className="text-slate-900" strokeWidth={2.5} />
              </div>
              <div className="leading-none">
                <span className="font-black text-white tracking-tight text-xl">Kit</span>
                <span className="font-black text-amber-500 tracking-tight text-xl">Certo</span>
              </div>
            </Link>
            <nav className="hidden md:flex items-center gap-6">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className="text-sm text-slate-400 hover:text-white transition-colors font-medium">{link.label}</Link>
              ))}
            </nav>
          ...
          {menuOpen && (
            <div className="md:hidden border-t border-slate-800 py-4 space-y-2">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className="block px-2 py-2 text-slate-300 hover:text-amber-500 font-medium transition-colors" onClick={() => setMenuOpen(false)}>{link.label}</Link>
              ))}
            </div>
          )}
        </div>
